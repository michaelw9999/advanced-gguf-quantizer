    ...
    if (transposed) {
        ...
        if (grid_y > USHRT_MAX || grid_z > USHRT_MAX) {
            ...
            ggml_cuda_kernel_launch(cpy_scalar_transpose<dst_t>, launch_params,
                cx, cdst, ne, ne00n, ne01n, ne02n, nb00, nb01, nb02, nb03,
                ne10, ne11, ne12, nb10, nb11, nb12, nb13);
        }
    } else {
        launch_scalar_generic();
...
// Check whether a same-type copy is a 2D strided copy of contiguous byte rows.
static bool ggml_cuda_cpy_as_memcpy_2d(
